...
CVSHEADER("$CVSHeader: cwg/rasputin/src/dg_scripts.c,v 1.7 2005/01/17 03:17:53 zizazat Exp $");
...
**/
        ...
        if (PLR_FLAGGED(c, PLR_THIEF))
          ...
          if (!str_cmp(field, "prac")) {
            if (subfield && *subfield) {
              int addition = atoi(subfield);
              GET_PRACTICES(c, GET_CLASS(c)) += addition;
            }
...
/* see note in dg_scripts.h */
        ...
        if (RIDDEN_BY(c))
          ...
          snprintf(str, slen, "0");
          script_log("Trigger: %s, VNum %d. Attempting to fetch obsolete value 'stradd'", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig));
        }
