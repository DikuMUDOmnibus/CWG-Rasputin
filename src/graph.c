...
CVSHEADER("$CVSHeader: cwg/rasputin/src/graph.c,v 1.3 2004/12/21 18:14:26 zizazat Exp $");
...
ACMD(do_track)
{
  ...
  if (IS_NPC(ch) || !GET_SKILL(ch, SKILL_KNOWLEDGE_WILDERNESS_LORE)) {
  ...
  if (!roll_skill(ch, SKILL_KNOWLEDGE_WILDERNESS_LORE)) {
    int tries = 10;
