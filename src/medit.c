...
CVSHEADER("$CVSHeader: cwg/rasputin/src/medit.c,v 1.8 2004/12/30 19:06:03 fnord Exp $");
...
void medit_disp_menu(struct descriptor_data *d)
{
  ...
  "@gC@n) Num HP Dice: [@c%4d@n],  @gD@n) Size HP Dice: [@c%4d@n],  @gE@n) HP Bonus: [@c%5d@n]\r\n"
  "@gF@n) Armor Class: [@c%4d@n],  @gG@n) Exp:     [@c%9d@n],  @gH@n) Gold:  [@c%8d@n]\r\n",
 ...
 */
  ...
  case MEDIT_LEVEL:
    GET_HITDICE(OLC_MOB(d)) = LIMIT(i, 1, 100);
    break;
