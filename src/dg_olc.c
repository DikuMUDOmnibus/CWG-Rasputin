...
CVSHEADER("$CVSHeader: cwg/rasputin/src/dg_olc.c,v 1.4 2004/12/30 19:06:03 fnord Exp $");
...
ACMD(do_oasis_trigedit)
{
  ...
  mudlog(BRF, ADMLVL_IMMORT, TRUE,"OLC: %s starts editing zone %d [trigger](allowed zone %d)",
         GET_NAME(ch), zone_table[OLC_ZNUM(d)].number, GET_OLC_ZONE(ch));
}
